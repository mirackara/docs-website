...
import queriesnrqlNrqlContextDemo from 'images/queries-nrql_screenshot-crop_nrql-context-demo.gif'
...
## Create a NRQL alert condition from an existing policy [#create-existing-policy]

To create a NRQL alert condition from an existing policy:

1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (policies)**.
2. Select an [existing policy](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/create-edit-or-find-alert-policy/#alert-policy-view). Note that editing an existing condition can result in [resetting its evaluation](#evaluation-resets).
3. Click **<Icon name="fe-plus-circle"/> Add a condition**.
...
5. Follow these steps.
    ...
    <Collapser
      ...
      1. Choose one of these options:
            ...
            * **Build a query**. Use NRQL to identify a signal. See [How to use NRQL: the mechanics of querying](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-how-nrql-works/) if you need help for adding a query.
            * **Golden signal or metric**. This option allows you to create NRQL alert conditions without knowing any NRQL. Follow and choose the options that adjust to your needs.
    ...
    <Collapser
        ...
        Click **Next** when you finish your configuration.

    </Collapser>
    <Collapser
        id="step3"
        title="Step 3. Define threshold"
        >
    
        You can define [thresholds](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/advanced-techniques/set-thresholds-alert-condition/) to determine what will open an incident. Once defined, click **Next**.
    ...
    <Collapser
        ...
        Add the details of the alert condition. Add a name to the alert to find an use it easily. Once you finish of adding a name and the additional settings, click **Save condition**.

    </Collapser>
    </CollapserGroup>

## Create a NRQL alert condition from a new policy [#create-new-policy]

To create a NRQL alert condition from a new policy:

1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (policies)**.
2. Add a policy name to find an use it easily.
3. Select an issue creation preference. You have these options to create issues and gorup incidents into them:

    * One issue per policy.
    * One issue per condition.
    * One issue per incident.

    Click the **Correlate and suppress noise** enable automatically [correlation](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/change-applied-intelligence-correlation-logic-decisions/).
4. Click **Set up notifications** to start receiving notifications or click **Create policy without notifications** if you don't want to receive notifications.
5. If you clicked **Set up notifications**, you can now create a new workflow to get notified or choose an existing workflow. See [Destinations](/docs/alerts-applied-intelligence/notifications/destinations/) to help you choose a channel.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * Add the runbook URL to include your organization's procedures for handling incidents. You may also add this information to the custom violation description.
      </td>
    </tr>
