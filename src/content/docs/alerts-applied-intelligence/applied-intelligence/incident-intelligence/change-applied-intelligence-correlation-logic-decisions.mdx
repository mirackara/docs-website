...
We offer two versions of the Decision Builder to create custom decision:

### [Basic Decision Builder](#Basic Decision Builder)
This version covers majority of use cases and focuses on "correlate by attribute," where you can specify filter conditions for correlation matches. You can also apply same filter logics for specific values to both incidents being correlated. For example, you can correlate incidents if the entity name is "host 1" for both.

### [Advanced Decision Builder](#Advanced Decision Builder)
In addition to the feature basic decision builder provide, advanced Decision Builder allows for more complex decision creation by applying different logic filters to two incidents being correlated. For example, you can correlate incidents if one has entity name "host 1" and the other has entity name "host 2”.


Our [algorithm](#what-is-correlaton) groups correlated incidents into a single issue, combining incidents with shared connections into one issue.

Let's dive into the Basic Decision Builder first.

### Basic Decision Builder

### Overview

A decision is composed of the following elements
...
* Correlation Time range: Sets the maximum time between two incidents created time for them to be eligible for correlation.
...
3. The second attribute usually matches the first, so it's autofilled for you. You can keep the autofilled option or choose another operator.
4. Click "Add" to include this filter. [Simulation](#simulations) will run automatically.
5. Repeat steps 1-4 to add up to 8 logic filters.
(Need a scrrenshot here)

#### Step 2: Filter by specific values
1. Click "See more options" to open the "Filter by specific values" section for additional filters.
...
6. Repeat steps 2-5 to add up to 8 logic filters.
(Need a scrrenshot here)

#### Step 3: Filter by related entities
1. Click "Filter by related entities" button.
...
Correlation Time Range is set to 20 minutes by default. You can adjust time range between 1 minute and 120 minutes (2 hours) to suit your specific needs.
(Need a scrrenshot here)

#### Step 5: Testing your rule with [simulation](#simulations)
1. After adding a filter logic, the system automatically runs a simulation using past 7 days of incident data.
2. You can also manually trigger simulation by clicking the "Simulate" button if anything changes in the rule.
(Need a scrrenshot here)

#### Step 6: Name and save your decision
1. Click "Create Decision" to access the name and description panel.
2. The system auto-generates a name based on your rule. Customize the name and description as desired.
3. Click "Save Decision" to finalize and save your new decision in the system.
(Need a scrrenshot here)

### Advanced Decision Builder

For more complex rule creation, click "Use Advanced Builder" to access the advanced decision builder and create custom rules for your specific requirements.
(Need a scrrenshot here)
