...
* `violation_chart_url` will have a value of `Not Available` if chart generated failed or did not return in a timely manner.
* The email sender will change to `noreply@notifications.newrelic.com`.

### incident_id
The `incident_id` on PagerDuty, Webhook, VictorOps, OpsGenie and xMatters notifications refers to the classic incident id. The classic incident id is deprecated, consumers should start to use the `issue_id` instead.  Previously, the `incident_id` was present on every notification. Now, it's not guaranteed to be added to every issue by the time the first notification is sent.
