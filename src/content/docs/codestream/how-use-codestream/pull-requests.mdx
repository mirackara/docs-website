...
<Callout variant="important">
  The pull request integration is available for GitHub, GitHub Enterprise, GitLab, GitLab Self-Managed, Bitbucket, and Bitbucket Server. Note that pull request review is not, however, available for Bitbucket Server.
</Callout>
...
Click the PR Details entry in the tree to manage the pull request. Change the pull request's reviewers, assignees, and labels, or add comments to the pull request that aren't code comments.
...
* Create a new section based on a custom query. Note that there are some special guidelines when creating a custom query for [GitHub and GitHub Enterprise](#github), or [GitLab and GitLab Self-Managed](#gitlab). Custom queries are not available for Bitbucket.
