...
The default select operation is a `COUNT`, but we also support `SUM` even for several metrics using a wildcard:
...
Another important differente in this example is the usage of wildcard (`%`) to query values of all metrics with the same prefix. Imagine we are reporting request count by status (e.g. `http.request.status.200.count`, `http.request.status.201.count`, `http.request.status.400.count`, ect...), the query will sum all matching metric name using the wildcard.
