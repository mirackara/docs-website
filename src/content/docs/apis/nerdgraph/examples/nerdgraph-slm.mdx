...
mutation {
  serviceLevelCreate(
    entityGuid: "MTAxMDAyNjh8TlIxfFdPUktMT0FEfDg3MTI5"
    indicator: {
      name: "test juan"
      description: "The proportion of valid requests that were served faster than 0.25s, which is considered to correspond to a good experience."
      events: {
        validEvents: { 
          select: {
            function: SUM
            attribute: "newrelic.goldenmetrics.apm.%"
          },
          from: "Metric"
        }
        goodEvents: {
          ...
          from: "Metric",
          where: "appName not like 'bad-app%'"
        }
        accountId: 10100268
      }
      objectives: {
...
Another important differente in this example is the usage of wildcard (`%`) to query values of all metrics with the same prefix, we are summing all the values from metrics with prefix `newrelic.goldenmetrics.apm.`.
