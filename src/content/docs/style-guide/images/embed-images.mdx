...
* Our default is dark mode. If an SME provides you with screenshots in light mode, please request new ones in dark mode. If you are unable to get dark mode images, then any screenshot is better than no screenshot at all. 
...
<CollapserGroup>
  <Collapser title={<>Embed image in <InlineCode>.mdx</InlineCode> file</>}>
    ...
    * Insert the image in the body of the document using the `img` tags.
      ...
      import syntheticsOverviewPage from 'images/synthetics_screenshot-full_overview-page.png'
  ...
  <Collapser title={<>Embed image in <InlineCode>.md</InlineCode> file</>}>
    ...
    ```md
    ![An image showing an overview of the synthetic monitoring UI](./images/synthetics_screenshot-full_overview-page.png 'Synthetics UI overview')
    ```
  </Collapser>
...
<CollapserGroup>
  ...
  ```
  import apmTransactionTimesChart from 'images/apm_screenshot-full_transaction-times-chart.png'
  ```
...
import exampleIcon from 'images/example_icon_example-icon.png'
