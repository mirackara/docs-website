...
<CollapserGroup>
  ...
  >
    ...
    However, some requests to services will generate many spans, and the agent span limit will be reached. As a result, some traces will not have full detail for that service. One way you can avoid this is to increase the span reservoir limit. See the configuration documentation for your APM agent.
