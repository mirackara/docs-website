...
<CollapserGroup>
  ...
  >
    An APM agent instance using head-based sampling has a maximum limit on the number of spans per minute per agent instance (the default limit is 2000). The agent attempts to keep all spans that are marked to be sampled as part of a distributed trace.
