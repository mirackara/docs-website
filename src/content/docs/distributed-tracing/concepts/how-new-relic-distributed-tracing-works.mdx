...
<CollapserGroup>
  ...
  >
    An APM agent instance using head-based sampling has a maximum limit on the number of spans per minute per agent instance (the default limit is 2000). The agent attempts to keep all spans that are marked to be sampled as part of a distributed trace. If you pass the limit, you could end up with [fragmented traces](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#fragmented-traces).
    ...
    However, some requests to services will generate many spans, and the agent span limit will be reached. As a result, some traces will not have full detail for that service. Here are two options to address this:

    * You could increase the span reservoir limit. See the configuration documentation for your APM agent.
    * You could create custom instrumentation so the agent reports less activity and therefore reports fewer spans.
...
Our [Infinite Tracing](/docs/distributed-tracing/infinite-tracing/introduction-infinite-tracing) feature uses a tail-based sampling approach. "Tail-based sampling" means that trace-retention decisions are done at the tail end of processing after all the spans in a trace have arrived.
