...
* Have a valid <LicenseKey/>.

To install infrastructure in Linux, follow these instructions:

1. Create the configuration file and add your <LicenseKey/>:
   ...
   <CollapserGroup>
     ...
     >
       ...
       <Callout variant="important">
         There's a known issue with the zypper package manager where GPG keys may not be validated as expected. If you get errors such as `Signature verification failed`, see New Relic's [Support Forum](https://discuss.newrelic.com/t/new-relic-infrastructure-installation/121241/3) for more information. The recommended alternative is to use `rpm --import` (insted of `gpg --import`) to import the gpg keys to the rpm trusted key database, Zypper will pick it up from there.
       </Callout>
     </Collapser>
   </CollapserGroup>
