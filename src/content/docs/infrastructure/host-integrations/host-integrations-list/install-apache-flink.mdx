...
You will need the Apache Flink prometheus jar file to expose metrics on a configured port. Ideally, on downloading the Apache Flink, the below file will be placed in the below path.
File path: `FLINK-DIRECTORY/plugins/metrics-prometheus/`
File name: `flink-metrics-prometheus-VERSION.jar`
...
File path: `FLINK-DIRECTORY/conf/flink-conf.yaml`
...
eg: `cluster_name: "apache_flink"`
urls: ["http://<YOUR_DOMAIN>:9250", "http://<YOUR_DOMAIN>:9251"]
eg: `urls: [“http://localhost:9250”, “http://localhost:9251”]`
