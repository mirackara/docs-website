...
1. Create a file named `nri-prometheus-config.yml` in `/etc/newrelic-infra/integrations.d` directory
...
On installing infrastructure agent on the linux machines, your log file named `logging.yml` should be present in the below path:
...
You can choose our pre-built dashboard template named Apache Flink to monitor your Apache Flink metrics. Follow these steps to use our pre-built dashboard template:
...
Below is an example query to view the total number of completed checkpoints on New Relic’s Query Builder
