...
There are different ways to scrape Prometheus metrics and forward them to NewRelic, check [our docs to know more](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent).
...
<CollapserGroup>
  ...
  >
    In some situations, when a high number of metrics are to be reported, `nri-prometheus` could time out while attempting to scrape the exporter. A corresponding error should be found in the [log file](/docs/infrastructure/new-relic-infrastructure/troubleshooting/generate-logs-troubleshooting-infrastructure):
    ...
    Consider increasing the default `scrape_timeout` value to mitigate this issue.
  </Collapser>
</CollapserGroup>
