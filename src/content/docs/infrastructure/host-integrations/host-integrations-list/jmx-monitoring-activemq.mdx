...
```
  ...
  >
This step organizes your metrics to be human readable once inside New Relic. Update the `jvm metrics.yml` file with any metrics that jconsole should capture. Below is the snippet of jvm-metrics.yml file.
    
Tip:
Use the jconsole tool to get the values of `YOUR_BROKER_NAME`, `YOUR_TOPIC_NAME`, `YOUR_QUEUE_NAME`
    
Note:
1. While using the below snippet, you're free to update the below snippet with your desired metrics.
2. Update `YOUR_BROKER_NAME`, `YOUR_TOPIC_NAME`, and `YOUR_QUEUE_NAME` with your actual values.
3. All the metrics captured are stored in `event_type`.

To update jvm-metrics.yml file:
