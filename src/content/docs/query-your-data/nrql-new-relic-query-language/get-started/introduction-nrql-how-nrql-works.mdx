...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Events and attributes that we report by default have names that contain alphanumeric characters, colons (`:`), periods (`.`), and underscores (`_`). Default-reported names also will start with an alphabetical letter, and won't have spaces. For custom names that don't meet these rules, you must use backticks ``` `` ``` around them. For example:
