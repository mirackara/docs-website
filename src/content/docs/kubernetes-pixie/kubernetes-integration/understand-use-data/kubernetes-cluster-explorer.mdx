...
import k8sNavigatorPreviewAlerts from 'images/k8s_navigator_preview_alerts.webp'

import k8sNavigatorPreviewFeedback from 'images/k8s_navigator_preview_feedback.webp'

import k8sNavigatorPreviewFiltering from 'images/k8s_navigator_preview_filtering.webp'

import k8sNavigatorPreviewOptions from 'images/k8s_navigator_preview_options.webp'

import k8sNavigatorPreviewPods from 'images/k8s_navigator_preview_pods.webp'

import k8sNavigatorPreviewAnimation from 'images/k8s_navigator_preview_animated.gif'
...
/>
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Kubernetes > your_cluster > Overview**: You can access the Kubernetes dashboard within the Kubernetes cluster explorer. It shows useful overview data about the status and performance of the cluster and its containerized workloads.
</figcaption>
...
## Kubernetes Navigator Preview


<Callout title="Preview">
  The Kubernetes Navigator UI is currently in preview. You can enable it using the `Preview the new Kubernetes Experience` toggle in the Kubernetes cluster explorer.
</Callout>

New Relic's Kubernetes Navigator UI builds upon the concept of [New Relic Navigator](https://docs.newrelic.com/docs/new-relic-solutions/new-relic-one/core-concepts/new-relic-explorer-view-performance-across-apps-services-hosts/#view-navigator) by enabling users to easily group entities and isolate high resource consumption, capacity issues, and alert status for Kubernetes Pods, Daemonsets, Deployments, Statefulsets, and Nodes.

Use the drop down filters to select an Entity Type, Metric, and Group by.  The preview supports the following entity types:
...
Here are some tips about how to use the Kubernetes Navigator:

* Use the drop down filters to select an **Entity Type**, **Metric**, and **Group by**.
    <img
...
* Capacity and utilization metrics are colored using a blue gradient which indicates whether a metric value is high (dark blue) or low (light blue/white).
    <img
...
* Use the filter bar or interact with the grouping headers to apply filters to scope the UI to a specific Node, Namespace, Deployment, etc.
...
* Hover over an entity to view its alert status and the value of the metric selected in the dropdown.  Click to open the flyout to view more details about the entity you've selected.  These details include tags, golden signals, logs, and access to more detailed entity dashboard pages via the `Details` links.
...
* The Kubernetes Navigator UI is in preview at the moment, and we'd love to get your feedback on what you liked and what other features you'd like to see.  You can submit feedback directly in the New Relic UI by visiting the **Help > Give Us Feedback** option in the bottom left nav.
