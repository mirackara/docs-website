...
<Callout variant="important">
  The Kubernetes Navigator UI is currently in public preview. Users may enable it using the `Preview the new Kubernetes Experience` toggle in the Kubernetes Cluster Explorer.
</Callout>  
...
Use the drop down filters to select an Entity Type, Metric, and Group by.  Supported Entity Types while in preview are:
