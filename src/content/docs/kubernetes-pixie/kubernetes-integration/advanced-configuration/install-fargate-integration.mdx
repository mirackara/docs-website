...
<Callout variant="important">
  New Relic Logging is not yet supported on Fargate nodes, but you can use the following as a workaround:
...
2. If a secret doesn't exist, create one in the same namespace as the pod containing the New Relic <LicenseKey/>, which is needed for the sidecar to report data.
...
4. Create a secret containing the New Relic <LicenseKey/>. Each namespace needs its own secret.
...
<Callout variant="important">
  The agent cannot hot load the New Relic <LicenseKey/>. After updating the secret, you need to rollout the deployments again.
</Callout>
