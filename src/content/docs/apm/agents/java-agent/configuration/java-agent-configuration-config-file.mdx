...
[New Relic Security agent interactive applications security testing](https://docs.newrelic.com/docs/iast/use-iast/) (IAST) mode.
...
New Relic Security agent configuration is set in the `security` stanza and can be [overridden](#System_Properties) by using a `newrelic.config.security` prefixed system property. Options include:
