...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Optional. Defaults to the <LicenseKey/> set in the New Relic agent's `newrelic.ini` file.

        Provide a different <LicenseKey/> if you want the transaction to report to a different New Relic account. If set, this license will supersede all per-directory and global default licenses configured in INI files.
      </td>
    </tr>
  </tbody>
</table>
...
This example manually specifies the app name and the <LicenseKey/>:
