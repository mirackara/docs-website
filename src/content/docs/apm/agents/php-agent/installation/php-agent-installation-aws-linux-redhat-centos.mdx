...
Our PHP agent auto-instruments your code so you can start monitoring applications. Follow this procedure to install New Relic's PHP agent for APM using Amazon Linux 2, RedHat, or CentOS. **Exception:** If you have an earlier version installed, [upgrade the agent](/docs/agents/php-agent/installation/upgrading-php-agent) instead.
...
1. Make sure you have your <LicenseKey/> accessible.
...
6. Generate traffic to your application, and wait a few minutes for it to send data to New Relic. Then, [check your app's performance in the UI](/docs/apm/applications-menu/monitoring/apm-overview-page).
