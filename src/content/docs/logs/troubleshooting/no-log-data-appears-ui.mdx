...
* Review our docs on setting up [APM logs-in-context](/docs/logs/logs-context/get-started-logs-context/)
* Make sure your APM agent is upgraded to a version that supports logs-in-context.
* Here is a list of supported agents and versions:
  ...
  * .NET logs in context (agent v9.7.0.0 or higher)
      * NLog: 4.1+ (.NET Framework) 4.5+ .NET Core requires .NET Agent v9.7.
      * Serilog: 2.0 or higher (.NET Framework), 2.5+ (.NET Core). Requires .NET Agent v9.7.0+.
...
* Infrastructure agent: [logging config options](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#logging-variables)
    ...
    <tr>
      ...
      <td>
        ...
        This error means that you're using an invalid security key. New Relic requires a <LicenseKey/> to enable log shipping.
